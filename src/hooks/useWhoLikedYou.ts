...
export const useWhoLikedYou = () => {
  ...
  const usersToDiscover: UserToDiscover[] = usersAccounts.data?.map(user => ({
    name: user.name,
    avatar: usersProfilePics?.find(pic => pic.user_id === user.id)?.urls?.[0] ?? "",
    liked: usersWhoLiked.data?.some(like => like.liked_by_user_id === user.id) ?? false,
    ringed: usersWhoRinged.data?.some(ring => ring.ring_by_user_id === user.id) ?? false,
  })) ?? [];

  const isLoading =
    usersWhoLiked.isLoading ||
    usersWhoRinged.isLoading ||
    usersAccounts.isLoading;

  const error =
    usersWhoLiked.error ||
    usersWhoRinged.error ||
    usersAccounts.error;
