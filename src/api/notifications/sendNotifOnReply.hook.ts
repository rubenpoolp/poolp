...
const useSendNotifOnReply = () => {
  ...
  return useMutation({
    ...
    mutationFn: async (userIds: string[]) => {
      ...
      const usersInCircle = await getLastInteractionFromCircleUsers(userIdsWithoutCurrentUser);
      ...
      return sendNotif(usersActiveSinceCircleCreation.map(user => user.id), i18n.t("notifications.onReply.title"), i18n.t("notifications.onReply.body", { username: auth.user?.name }));
    },
    onError: (error: Error) => {
    ...
    onSuccess: () => {
      ...
      console.log("Notification to users in circle sent");
    },
  });
};
